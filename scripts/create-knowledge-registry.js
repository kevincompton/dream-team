#!/usr/bin/env node
/**
 * Creates an HCS-2 indexed registry for Knowledge topics (Validator-created HIP-991 topics).
 * Run once to bootstrap; set KNOWLEDGE_REGISTRY_TOPIC_ID in .env.
...
async function main() {
  ...
  if (!operatorId || !operatorKey) {
    throw new Error("VALIDATOR_ACCOUNT_ID and VALIDATOR_PRIVATE_KEY (or HEDERA_ACCOUNT_ID / HEDERA_PRIVATE_KEY) required");
  }
  ...
  console.log(`Creating HCS-2 indexed Knowledge registry on ${network}...`);
  ...
  console.log(`✓ HCS-2 Knowledge registry created: ${topicId}`);
  console.log(`  Indexed — full history, update/delete supported`);

  let envRaw = fs.existsSync(ENV_PATH) ? fs.readFileSync(ENV_PATH, "utf8") : "";
  envRaw = upsertEnvValue(envRaw, "KNOWLEDGE_REGISTRY_TOPIC_ID", topicId);
  fs.writeFileSync(ENV_PATH, `${envRaw.trimEnd()}\n`, "utf8");

  console.log(`✓ .env updated: KNOWLEDGE_REGISTRY_TOPIC_ID=${topicId}`);
}
